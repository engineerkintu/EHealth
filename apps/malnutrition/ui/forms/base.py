...
from django.utils.encoding import smart_text, python_2_unicode_compatible, force_text
from django.utils.safestring import mark_safe
        
def as_div(self):
    if not self: return u''
    return mark_safe(u'<span class="field-error">%s</span>'
            % ''.join([u'%s<br />' % conditional_escape(force_text(e)) for e in self]))
            
forms.utils.ErrorList.__unicode__ = as_div